Feature: `start-server` command
  ...
  Scenario: Starting the service with a provided port number that is already in use
    Given a test service is started on port 8134
    When I run `freshli-agent-java start-server 8134`
    Then the output should contain:
    """
    Unable to start the gRPC service. Port 8134 is in use.
    """
    And the exit status should not be 0
    When the test service running on port 8134 is stopped
    Then there are no services running on port 8134
