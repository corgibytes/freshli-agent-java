Feature: `retrieve-release-history` command
  This command outputs the full release history for a package. The package is specified using a
  [PURL or Package URL](https://github.com/package-url/purl-spec).

  This command's output is tab-delimited to assist with machine parsing of the output. Rows are sorted by date, with
  the version component coming first. Dates are displayed using ISO8601 format.
  ...
  Scenario: Valid Package URL from alternative repository
    ...
    When I run `freshli-agent-java retrieve-release-history pkg:maven/org.springframework/spring-core?repository_url=repo.spring.io%2Frelease`
    Then the output should contain:
    """
  ...
  Scenario: Valid Package URL from alternative repository that includes url scheme
    ...
    When I run `freshli-agent-java retrieve-release-history pkg:maven/org.springframework/spring-core?repository_url=http%3A%2F%2Frepo.spring.io%2Frelease`
    Then the output should contain:
    """
    1.0	2005-12-23T17:06:05Z
    1.0-rc1	2005-12-23T17:06:05Z
  ...
  Scenario: Valid Package URL for an unknown package
    ...
    When I run `freshli-agent-java retrieve-release-history pkg:maven/com.corgibytes/missing`
    Then the output should contain:
    """
    Unable to find release history for pkg:maven/com.corgibytes/missing.
    """
  ...
  Scenario: Invalid Package URL
    ...
    When I run `freshli-agent-java retrieve-release-history invalid`
    Then the output should contain:
    """
    Unable to parse the Package URL: invalid.
    """
    And the exit status should not be 0
